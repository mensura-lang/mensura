/-
The operations of the data-handling algebra over indexed tables: `map` (the
single row-wise primitive), the fixed-right joins, `aggregate`, `ungroup`,
`project`, and the tagged bind/split pair with its reversibility.
...
`map` subsumes def:selection, def:mutating, def:filtering, and the
