...
\begin{theorem}[Map is split-safe]
  ...
  \uses{def:map, def:splitsafe}
  Every map is split-safe.
\end{theorem}
\begin{proof}\leanok
  A map distributes over every bind (multiset bind distributes over
...
\begin{theorem}[The drop variant is split-safe]
  ...
  distributes over every bind, hence is split-safe.
\end{theorem}
\begin{proof}\leanok
...
\begin{theorem}[Project inverts ungroup]
  \label{thm:project-ungroup}\lean{Mensura.project_ungroup}\leanok
  \uses{def:ungroup, def:project}
  ...
  \texttt{ungroup}), demoting the component again restores the table
  exactly.  The \texttt{extend\_key}/\texttt{shrink\_key} round trip is
  the identity, hence preserves every tracked property at once.
\end{theorem}
\begin{proof}\leanok
  Push the tagging map into the fiber bind, exchange the finite sum with
  the bind, and each row collapses to the singleton of itself: the tag
  it receives is the key component it was filed under.
\end{proof}

\begin{theorem}[Ungroup inverts project]
  \label{thm:ungroup-project}\lean{Mensura.ungroup_project}\leanok
  \uses{def:ungroup, def:project}
  ADR 0024, with no side condition: \texttt{project} tags every demoted
...
\end{theorem}
\begin{proof}\leanok
  Distribute the bind over the fiber sum; at each fine key the tag
...
\begin{theorem}[A presence-preserving fiber map preserves the rectangle]
  ...
  \ref{thm:aggregate-exhaustive} and \ref{thm:map-exhaustive} are the two
  principal cases.
\end{theorem}
\begin{proof}\leanok
  Presence is preserved in both directions.
