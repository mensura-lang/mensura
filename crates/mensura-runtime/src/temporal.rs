...
/// The whole-millisecond predicate of ADR 0036 decision 6, shared with the
/// resolver's compile-time `lateness` check (ADR 0037): one implementation,
/// so the two sides of the boundary agree on the tolerance to the digit.
pub use mensura_types::temporal::whole_milliseconds;
